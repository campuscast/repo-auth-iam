...
WORKDIR /workspace/repo-auth-iam
COPY repo-auth-iam/package*.json ./
...
COPY --from=builder /workspace/repo-auth-iam/dist ./dist
COPY --from=builder /workspace/repo-auth-iam/node_modules ./node_modules
COPY --from=builder /workspace/repo-auth-iam/package.json ./
USER appuser
EXPOSE 3000
